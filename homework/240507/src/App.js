...
// images
import designLogo from "./images/fe.png";
import frontLogo from "./images/gd.png";
import backLogo from "./images/be.png";
...
function App() {
  ...
  const [selectedLionName, setSelectedLionName] = useState("");
  ...
  const selectLion = (id) => {
    setSelectedId(id);
    const lion = trackData.find((lion) => lion.id == id);
    setSelectedLionName(lion?.name || ""); // 찾은 사용자의 이름을 설정하거나, 찾지 못했으면 빈 문자열로 설정
  };
  ...
  const selectTrack = (selectedTrack) => {
    ...
    setSelectedId(null); // 선택된 사용자 ID 초기화
    setSelectedLionName(""); // 선택된 사용자 이름 초기화
  };
  ...
  return (
    <Wrapper>
      <BackgroundModal>
        ...
        <OpenBtn onClick={openModal}>모달 만들기</OpenBtn>
